  ...
  <div class="container">
    ...
    <mt-tab-container v-model="selected" class="order-wrap">
          ...
          <div v-for="(d, index) in all_list" :key="d.orderNo" class='cell-margin' @click="goDetail(d)">
            <mt-cell>
              ...
              <div v-if="d.orderStatus == '1'">
                <mt-button type="danger" size="small" class="pay-b" @click.stop="cancelOrder(d)">取消订单</mt-button>
      ...
      <mt-tab-container-item id="to_pay">
        <mt-loadmore :auto-fill="false" :top-method="getToPayList" :bottom-method="getToPayList" :bottom-all-loaded="allLoaded_2" ref="loadmore2">
          <div v-for="(d,index) in to_pay_list" :key="d.orderNo" class='cell-margin' @click="goDetail(d)">
            <mt-cell>
              <div slot="title" class="order-des">
                <div>
                  <label>状态：</label><span style="color: #ef4f4f;">待支付</span></div>
                <div>
                  <label>总价：</label><span>￥{{d.orderAmount}}</span></div>
              </div>
              <div>
                <mt-button type="danger" size="small" class="pay-b" @click.stop="cancelOrder(d, index)">取消订单</mt-button>
                <mt-button type="primary" size="small" class="pay-b" @click.stop="goPay(d)">去支付</mt-button>
              </div>
            </mt-cell>
            <template v-for='i in d.productList'>
        ...
        <mt-loadmore :auto-fill="false" :top-method="getToReceiveList" :bottom-method="getToReceiveList" :bottom-all-loaded="allLoaded_3" ref="loadmore3">
          <div v-for="d in to_receive_list" :key="d.orderNo" class='cell-margin' @click="goDetail(d)">
            <mt-cell>
              <div slot="title" class="order-des">
                <div>
                  <label>状态：</label><span class="order-status">
                <template v-if="d.orderStatus == '3'">已支付</template>
                <template v-if="d.orderStatus == '4'">已发货</template>
              </span></div>
                <div>
...
<script>
import { reqBuyerOrderList, reqBuyerOrderCancel } from '../../api'
...
export default {
  ...
  methods: {
        ...
        reqBuyerOrderCancel({
          ...
          if (res.data.code == 0) {
            ...
            if (typeof i != 'undefined') {
              this.to_pay_list.splice(i, 1)
            }
            if (this.selected == 'all_orders' && this.to_pay_list.length > 0) {
              this.to_pay_page = 0
              this.to_pay_list = []
            } else if (this.selected == 'to_pay' && this.all_list.length > 0) {
              this.all_page = 0
              this.all_list = []
            }
          } else {
    ...
    getToReceiveList() {
      this.showSpinner()
      let obj = Object.assign({}, this.pa, { page: this.to_receive_page, orderStatus: '3,4' })
      reqBuyerOrderList(obj).then((res) => {
        if (res.data.code == 0) {
          for (let i of res.data.data.content) {
