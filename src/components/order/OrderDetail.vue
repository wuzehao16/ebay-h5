<template>
  <div class="container">
    <template v-if=' Number.parseInt(order.orderStatus) >= 3 '>
      <mt-cell :title='logistics[0].trackingMessage' v-if="logistics[0] && logistics[0].trackingMessage" :label='logistics[0].trackingTime' :is-link="true" @click.native='goLogi' style="padding-top: 10px;">
      </mt-cell>
    ...
    <mt-cell>
      <ul slot='title' class="wrap-one just-spe">
        ...
        <li><i>含税费：</i><span class="high-light">￥{{ taxFee }}</span></li>
        <li><i>含运费：</i><span class="high-light">￥{{ cFee }}</span></li>
      </ul>
    </mt-cell>
    <mt-cell>
